...
def render_room_program_images(program_json_data_path, image_save_folder="", load_progress=True):
        ...
        if load_progress:
            image_program_json_data = json.load(open("/projectnb/ivc-ml/array/research/robotics/dreamworlds/custom_datasets/procThor/procthor_roomjson_programs_imgs_sparsepanorama.json"))
        else:
            image_program_json_data = []
        
        
        for (program_text, house_json, og_house_json, cam_ind_to_position, all_imgs, all_objs, all_seg_frames, color_to_objid, obj_id_to_name) in tqdm.tqdm(program_json_data):
            ...
            '''
            #pdb.set_trace()
            json.dump(image_program_json_data, open("/projectnb/ivc-ml/array/research/robotics/dreamworlds/custom_datasets/procThor/procthor_roomjson_programs_imgs_sparsepanorama.json", "w"))
            controller.stop()
