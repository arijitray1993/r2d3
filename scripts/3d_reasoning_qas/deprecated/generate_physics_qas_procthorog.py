    ...
    for house in dataset["train"]:
        ...
        for cam_pos in random.sample(reachable_positions, 10):
            ...
            if len(nav_visible_objects) > vis_obj_count:
                ...
                max_vis_cam_pos = (cam_pos, cam_rot)
        
        if max_vis_cam_pos is None:
            print("No objects visible, continuing")
            controller.stop()
            continue

        cam_pos, cam_rot = max_vis_cam_pos
        
        controller.stop()

        
        for sample_i in range(2):
            controller = Controller(scene=house, width=800, height=800, quality="Ultra")
            try:
                controller.step(action="Teleport", position=cam_pos, rotation=cam_rot)
            except:
                print("Cannot teleport, continuing")
