...
if __name__=="__main__":
    ...
    for ind, room_data in enumerate(tqdm.tqdm(image_program_data)):
      ...
      prompt = "We need to make the AI generate a 3D model of the room as close to this image as possible. How would you instruct the AI agent to do this? These are some of the objects present in the view: " + ", ".join(objs) + ". Avoid using absolute locations like left-side or right-side wall since these can change depending on the perspective. Try to use the relative locations to a few other objects in the room to convey where things should roughly be placed. Just output a brief description without any other text."
