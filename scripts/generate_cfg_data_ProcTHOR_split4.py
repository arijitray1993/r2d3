...
def render_room_program_images(program_json_data_path, image_save_folder="", load_progress=True):
        ...
        if load_progress:
            image_program_json_data = json.load(open("/projectnb/ivc-ml/array/research/robotics/dreamworlds/custom_datasets/procThor/procthor_roomjson_programs_imgs_train_childrenadded_split4.json"))
        else:
        ...
        for ind, (program_text, house_json, og_house_json) in enumerate(tqdm.tqdm(program_json_data)):
            ...
            '''
            #pdb.set_trace()
            json.dump(image_program_json_data, open("/projectnb/ivc-ml/array/research/robotics/dreamworlds/custom_datasets/procThor/procthor_roomjson_programs_imgs_train_childrenadded_split_4.json", "w"))
            controller.stop()
